  ...
  static async createCheckoutSession(req, res, next) {
    try {
      ...
      if (carPriceInSen > maxStripeAmountIDR) {
        throw { 
          status: 400, 
          message: `Harga mobil (Rp ${car.price.toLocaleString('id-ID')}) melebihi batas maksimum Stripe IDR 2,000,000,000. Silakan hubungi support untuk metode pembayaran alternatif.` 
        };
      }

      // Validate minimum amount (Stripe IDR requires at least IDR 7,000)
      if (carPriceInSen < 700000) { // IDR 7,000 in sen
        throw { 
          status: 400, 
